...
OUTPUTS_DIR = BASE_DIR / "code" / "outputs"

MAJORS_FILE = DATA_DIR / "majors_dataset.xlsx"
...
@st.cache_data
def load_majors():
    return pd.read_excel(MAJORS_FILE, sheet_name="Majors")


try:
    majors = load_majors()
except FileNotFoundError:
    st.error(f"Majors dataset not found. Expected file path: {MAJORS_FILE}")
    st.stop()
...
def validate_student(student):
    ...
    if student["GPA_Min"] >= 4.8 and sum(1 for s in skills if s == 1) >= 4:
        warnings.append("Possible inconsistency: GPA is very high but most skills are marked Low.")
...
with col_save_profile:
    if st.button("💾 Save Student Profile"):
        if warnings:
            ...
            if success:
                st.success("Student profile saved successfully in code/outputs/new_students_profiles.xlsx")
            else:
                st.error(error)
...
if "top3" in st.session_state and not warnings:
    ...
    if st.button("✅ Save Feedback"):
        if warnings:
            ...
            if success:
                st.success("Feedback saved successfully in code/outputs/recommendation_feedback.xlsx")
            else:
                st.error(error)
